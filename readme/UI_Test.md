...
| 层次 | 组件 | 触发方式 | 用途 |
|------|------|----------|------|
| GM 指令 | `TestUIBoot` + `TestGMBoxPanel` | F1 打开面板，输入指令 | 运行时手动按需打开/关闭面板 |
| 手动调试 GUI | `UIFlowTest` | **F3** 切换 | IMGUI 悬浮窗，实时状态 + 快速操作 + 逐步执行 TC |
| 自动化测试 | `UIFlowTest` | **F2** 或 `uitest` | 自动顺序执行 6 条 TC，输出 PASS / FAIL |
...
├── TestUIID.cs             UI ID 常量表（GMBoxPanel=1 / TestPanel=2 / TestModal=3）
├── GMBoxPanel.cs           GM Box 的 UIManager 适配层（AbstractPanel 包装）
├── TestPanel.cs            通用测试面板（AbstractPanel，Auto 层）
...
Editor/Scripts/Test/
└── CreateUITestPrefabEditor.cs   一键生成全部 3 个测试 Prefab
...
一键在 `Assets/Package/ui/uiprefab/` 下生成以下三个 Prefab：

| Prefab | 挂载组件 | 层级 | 说明 |
|--------|----------|------|------|
| `ui_panel_gm_box.prefab` | `GMBoxPanel` | Top | 打开时同步显示 IMGUI GM 窗口 |
| `ui_panel_test.prefab` | `TestPanel` | Auto | 基础开关 / 缓存 / 计数测试 |
| `ui_panel_test_modal.prefab` | `TestModalPanel` | Auto | 打开后下层全隐藏且不可交互 |

> 也可单独生成：**ST.Core / Test / Create ui_panel_gm_box Prefab** 等子菜单。
...
> 所有引用（TitleText / CloseButton）均在生成时自动绑定，无需手动连线。
...
| ID | 常量名 | Prefab | 层级 | 备注 |
|----|--------|--------|------|------|
| 1 | `GMBoxPanel` | `ui_panel_gm_box.prefab` | Top | 打开时同步显示 IMGUI GM 窗口 |
| 2 | `TestPanel` | `ui_panel_test.prefab` | Auto | 基础测试面板 |
| 3 | `TestModal` | `ui_panel_test_modal.prefab` | Auto | 模态面板，HideMask=HideAndUnInteractive |
...
├───────────────────────────────────────────────────────────┤
│ ■ 分步执行测试用例                                         │
│  [TC01][TC02][TC03][TC04][TC05][TC06]           [重置]    │
│  ─────────────────────────────────────────────────────    │
│  TC01 基础开关流程  [2/4 步]                               │
│  ✓  1. 清理：关闭 TestPanel           IsOpened=false      │
│  ✓  2. OpenPanel(TestPanel)           IsOpened=true       │
│  ►  3. 等待 Prefab 异步加载完成        FindPanel≠null  [▶执行]│
│     4. ClosePanel(TestPanel)          IsOpened=false      │
├───────────────────────────────────────────────────────────┤
│ ■ 操作日志                                       [清空]   │
│  [TC01] Step1 PASS  IsOpened=false                        │
│  [TC01] Step2 PASS  IsOpened=true                         │
└───────────────────────────────────────────────────────────┘
```
...
直接调用 `UIManager.S.OpenPanel` / `ClosePanel`，结果写入日志区。

**分步执行区**

| 元素 | 行为 |
|------|------|
| `[TC01]…[TC06]` | 点击切换 TC，不重置已完成步骤（可继续上次进度）|
...
{
    public const int GMBoxPanel = 1;   // ui_panel_gm_box.prefab       (Top)
    public const int TestPanel  = 2;   // ui_panel_test.prefab          (Auto)
    public const int TestModal  = 3;   // ui_panel_test_modal.prefab    (Auto, HideMask)
}
```
...
运行后执行 `openui 4` 或在手动 GUI 快速操作区点击按钮验证。
...
```csharp
yield return TC07_MyCustomCase();

IEnumerator TC07_MyCustomCase()
{
    bool passed = true;
    Print("── TC07 自定义测试");
...
在 `InitManualTCs()` 的数组中追加 `BuildManualTC07()`：

```csharp
ManualTC BuildManualTC07()
{
    return new ManualTC("TC07 自定义", "TC07", new Step[]
    {
...
> `waitLoad: true` 表示该步骤执行后会自动轮询 `check()` 直到返回 `true` 或超时。
...
2. **Prefab 根节点必须有 Canvas**：`AbstractPanel` 通过 `[RequireComponent(typeof(Canvas))]` 强制要求，编辑器工具已自动添加。
3. **异步加载超时**：`WaitForPanel` 与手动步骤均默认等待 **5 秒**；低性能机器可修改 `UIFlowTest.k_LoadTimeout`。
4. **openCount 跨测试累积**：缓存复用时 `openCount` 不归零，TC04 通过记录初始值再比对增量来断言，而非直接比对绝对值。
5. **TC05 调用 DoClose**：会清空所有运行中的面板（但不清除 `UIDataTable` 静态注册），后续 TC 可正常继续。
6. **手动 GUI 与 UGUI 层级**：IMGUI 默认渲染在 UGUI 上方，手动 GUI 窗口不受 UIManager 层级管理影响。
7. **GMBoxPanel 与 TestGMBoxPanel 关系**：`GMBoxPanel`（UGUI）是 UIManager 适配层，实际 GM 功能由场景中的 `TestGMBoxPanel`（IMGUI）提供。两者通过 `Show()` / `Hide()` 接口联动。
